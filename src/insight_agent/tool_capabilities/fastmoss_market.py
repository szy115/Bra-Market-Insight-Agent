...
def load_fastmoss_market_product_specs() -> tuple[dict[str, Any], ...]:
    payload = json.loads(FASTMOSS_MARKET_PRODUCT_SPEC_PATH.read_text(encoding="utf-8"))
    specs = tuple(dict(item) for item in payload if isinstance(item, dict))
    names = tuple(str(item.get("name") or "") for item in specs)
    if names != FASTMOSS_MARKET_PRODUCT_MCP_TOOL_NAMES:
        raise ValueError("FastMoss market/product capability specs are incomplete or reordered")
    return specs
...
def fastmoss_market_product_catalog() -> dict[str, dict[str, Any]]:
    ...
    for spec in load_fastmoss_market_product_specs():
...
def build_fastmoss_market_product_capabilities(
    *,
    ...
    for capability_id, tool_meta in fastmoss_market_product_catalog().items():
